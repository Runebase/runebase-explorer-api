export class ContractService {
  ...
  async getContractSummary(contractAddress, addressIds, fromBlock, toBlock, transaction) {
    const { Contract, Qrc20: QRC20, Qrc20Statistics: QRC20Statistics, Qrc721: QRC721 } = this.db
    ...
    let [
      ...
      qrc20Service.getAllQRC20Balances([contractAddress], transaction),
      qrc721Service.getAllQRC721Balances([contractAddress], transaction),
      this.getContractTransactionCount(contractAddress, addressIds, fromBlock, toBlock, transaction)
    ])
    return {
  ...
  async getContractBasicTransactionCount(contractAddress, fromBlock, toBlock, transaction) {
    const { EvmReceipt: EVMReceipt } = this.db
  ...
  async searchLogs({ contract, topic1, topic2, topic3, topic4 } = {}, pagination, fromBlock, toBlock, transaction) {
    ...
    let [{ count: totalCount }] = await this.db.sequelize.query(sql`
    ...
    let ids = (await this.db.sequelize.query(sql`
      ...
      LIMIT ${offset}, ${limit}
    `, { type: this.db.sequelize.QueryTypes.SELECT, transaction })).map(log => log._id)
