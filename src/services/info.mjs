export class InfoService {
  ...
  async getStakeWeight(transaction) {
    const { Header } = this.db
    const { gte: $gte } = this.db.Sequelize.Op
    let height = await Header.aggregate('height', 'max', { transaction })
  ...
  async getFeeRates() {
    ...
    return [
      { blocks: 2, feeRate: results[0].feerate || 0.4 },
      { blocks: 4, feeRate: results[1].feerate || 0.4 },
      { blocks: 6, feeRate: results[2].feerate || 0.4 },
      { blocks: 10, feeRate: results[3].feerate || 0.4 },
      { blocks: 12, feeRate: results[4].feerate || 0.4 },
      { blocks: 24, feeRate: results[5].feerate || 0.4 }
    ]
  }
