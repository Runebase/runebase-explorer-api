...
export class MiscService {
  ...
  async classify(id, transaction) {
      ...
      where: {
        [$or]: [
          this.db.sequelize.where(
            this.db.sequelize.fn('LOWER', this.db.sequelize.fn('CONVERT', this.db.sequelize.literal('name USING utf8mb4'))),
            id.toLowerCase()
          ),
          this.db.sequelize.where(
            this.db.sequelize.fn('LOWER', this.db.sequelize.fn('CONVERT', this.db.sequelize.literal('symbol USING utf8mb4'))),
            id.toLowerCase()
          )
        ]
      },
    ...
    if (qrc20Results.length) {
      let [{ addressHex }] = await this.db.sequelize.query(sql`
