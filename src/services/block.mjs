export class BlockService {
  ...
  async getBlock(arg, transaction) {
    const { Header, Address, Block, Transaction } = this.db
    let filter
  ...
  async getRawBlock(arg, transaction) {
    ...
    let block = await Header.findOne({ where: filter, transaction })
    if (!block) {
      return null
    }
  ...
  async listBlocks(dateFilter, pagination, transaction) {
    ...
    if (blocks.length === 0) {
      ...
      return { totalCount, blocks: await this.getBlockSummary(blocks, transaction) }
    }
  }
  ...
  async getBiggestMiners(lastNBlocks, pagination, transaction) {
    ...
    const { sql } = this
    const { Block } = this.db
    const { gte: $gte } = this.db.Sequelize.Op
  ...
  getBlockFilter(fromBlock, toBlock, category = 'blockHeight') {
    const { gte: $gte, lte: $lte, between: $between } = this.db.Sequelize.Op
  ...
  getRawBlockFilter(fromBlock, toBlock, category = 'block_height') {
    ...
    if (fromBlock != null && toBlock != null) {
      blockFilter = this.sql`${{ raw: category }} BETWEEN ${fromBlock} AND ${toBlock}`
    } else if (fromBlock != null) {
      blockFilter = this.sql`${{ raw: category }} >= ${fromBlock}`
    } else if (toBlock != null) {
      blockFilter = this.sql`${{ raw: category }} <= ${toBlock}`
    }
