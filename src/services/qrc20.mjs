export class QRC20Service {
  ...
  async listQRC20Tokens(pagination, transaction) {
    const { Qrc20Statistics: QRC20Statistics } = this.db
    const sql = this.sql
  ...
  async getQRC20BalanceHistory(addresses, tokenAddress, pagination, transaction) {
    ...
    const { ne: $ne, and: $and, or: $or, in: $in } = this.db.Sequelize.Op
    ...
    let transactions = list.map(({ blockHeight, header, transaction: tx, logs }) => {
      let result = {
        id: tx.id,
        block: {
          hash: header.hash,
  ...
  async getQRC20TokenRichList(contractAddress, pagination, transaction) {
    const { Qrc20Balance: QRC20Balance } = this.db
    const { ne: $ne } = this.db.Sequelize.Op
    let { limit, offset } = pagination
  ...
  async updateQRC20Statistics() {
    ...
    try {
      ...
      await this.db.sequelize.query(sql`DELETE FROM qrc20_statistics`, { transaction })
