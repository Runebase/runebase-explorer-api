export class TransactionService {
  ...
  async getTransaction(id, transaction) {
    ...
    return {
      ...
      ...tx.contractSpendSource ? { contractSpendSource: tx.contractSpendSource.destTransaction.id } : {},
      contractSpends,
      size: tx.size,
      weight: tx.weight
    }
  ...
  async getRecentTransactions(count = 10, transaction) {
    ...
    const { or: $or, gt: $gt, lte: $lte } = this.db.Sequelize.Op
  ...
  async getAllTransactions(address, pagination, transaction) {
    ...
    if (address) {
      ...
      let totalCount = await Transaction.count({ transaction }) - (
  ...
  async getBasicTransaction(transactionId, addressIds, transaction) {
    const { Header, Transaction, TransactionOutput, TransactionInput, GasRefund, EvmReceipt: EVMReceipt } = this.db
    const { where, col } = this.db.sequelize
