...
export default (router) => {
  router.get('/qrc20', asyncHandler(async (req, res) => {
    let { totalCount, tokens } = await services.qrc20.listQRC20Tokens(req.state.pagination, req.state.transaction)
    res.json({
      totalCount,
  ...
  router.get('/qrc20/txs', asyncHandler(async (req, res) => {
    let { totalCount, transactions } = await services.qrc20.getAllQRC20TokenTransactions(req.state.pagination, req.state.transaction)
    res.json({
      totalCount,
  ...
  router.get('/qrc20/:token/txs', asyncHandler(async (req, res) => {
    ...
    let { totalCount, transactions } = await services.qrc20.getQRC20TokenTransactions(req.state.token.contractAddress, req.state.pagination, req.state.fromBlock, req.state.toBlock, req.state.transaction)
    res.json({
      totalCount,
      transactions: transactions.map(transaction => ({
        transactionId: transaction.transactionId.toString('hex'),
  ...
  router.get('/qrc20/:token/rich-list', asyncHandler(async (req, res) => {
    if (req.state.token.type !== 'qrc20') return res.status(404).end()
    let { totalCount, list } = await services.qrc20.getQRC20TokenRichList(req.state.token.contractAddress, req.state.pagination, req.state.transaction)
    res.json({
      totalCount,
      list: list.map(item => ({
