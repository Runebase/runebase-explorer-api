...
export default (router) => {
  ...
  router.get('/address/:address/balance/total-received', asyncHandler(async (req, res) => {
    let { totalReceived } = await services.balance.getTotalBalanceChanges(req.state.address.addressIds, req.state.transaction)
    res.send(totalReceived.toString())
  }))

  router.get('/address/:address/balance/total-sent', asyncHandler(async (req, res) => {
    let { totalSent } = await services.balance.getTotalBalanceChanges(req.state.address.addressIds, req.state.transaction)
  ...
  router.get('/address/:address/qrc20-balance/:token', asyncHandler(async (req, res) => {
    let { address, token } = req.state
    if (token.type !== 'qrc20') {
      return res.json({})
    }
    let { name, symbol, decimals, balance, unconfirmed } = await services.qrc20.getQRC20Balance(address.rawAddresses, token.contractAddress, req.state.transaction)
    res.json({
      name,
      symbol,
      decimals,
  ...
  router.get('/address/:address/txs', asyncHandler(async (req, res) => {
    let { address } = req.state
    let { totalCount, transactions } = await services.address.getAddressTransactions(address.addressIds, address.rawAddresses, req.state.pagination, req.state.fromBlock, req.state.toBlock, req.state.transaction)
    res.json({
      totalCount,
  ...
  router.get('/address/:address/basic-txs', asyncHandler(async (req, res) => {
    let { totalCount, transactions } = await services.address.getAddressBasicTransactions(req.state.address.addressIds, req.state.pagination, req.state.fromBlock, req.state.toBlock, req.state.transaction)
    res.json({
      totalCount,
  ...
  router.get('/address/:address/contract-txs', asyncHandler(async (req, res) => {
    let { address, contract } = req.state
    let { totalCount, transactions } = await services.address.getAddressContractTransactions(address.rawAddresses, contract, req.state.pagination, req.state.fromBlock, req.state.toBlock, req.state.transaction)
    res.json({
      totalCount,
      transactions: transactions.map(transaction => ({
  ...
  router.get('/address/:address/qrc20-txs/:token', asyncHandler(async (req, res) => {
    let { address, token } = req.state
    let { totalCount, transactions } = await services.address.getAddressQRC20TokenTransactions(address.rawAddresses, token, req.state.pagination, req.state.transaction)
    res.json({
      totalCount,
      transactions: transactions.map(transaction => ({
        transactionId: transaction.transactionId.toString('hex'),
  ...
  router.get('/address/:address/balance-history', asyncHandler(async (req, res) => {
    let { totalCount, transactions } = await services.balance.getBalanceHistory(req.state.address.addressIds, {}, req.state.pagination, req.state.transaction)
    res.json({
      totalCount,
  ...
  router.get('/address/:address/qrc20-balance-history', asyncHandler(async (req, res) => {
    ...
    let hexAddresses = req.state.address.rawAddresses
      .filter(address => address.type === Address.PAY_TO_PUBLIC_KEY_HASH)
      .map(address => address.data)
    let { totalCount, transactions } = await services.qrc20.getQRC20BalanceHistory(hexAddresses, tokenAddress, req.state.pagination, req.state.transaction)
    res.json({
      totalCount,
      transactions: transactions.map(tx => ({
        id: tx.id.toString('hex'),
