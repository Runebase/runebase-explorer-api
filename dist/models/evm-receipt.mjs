...
export default (sequelize, Sequelize) => {
  ...
  EvmReceipt.associate = models => {
    const {
      Header,
      Transaction,
      TransactionOutput
    } = models;
    EvmReceipt.belongsTo(Header, {
      as: 'header',
      foreignKey: 'blockHeight'
    });
    Transaction.hasMany(EvmReceipt, {
      as: 'evmReceipts',
      foreignKey: 'transactionId'
    });
    EvmReceipt.belongsTo(Transaction, {
      as: 'transaction',
      foreignKey: 'transactionId'
    });
    TransactionOutput.hasOne(EvmReceipt, {
      as: 'evmReceipt',
      foreignKey: 'transactionId'
    });
    EvmReceipt.belongsTo(TransactionOutput, {
      as: 'output',
      foreignKey: 'transactionId'
    });
  };
