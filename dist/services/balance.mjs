export class BalanceService {
  ...
  async getStakingBalance(ids, transaction) {
    ...
    let result = await TransactionOutput.aggregate('value', 'SUM', {
      where: {
        ...
        blockHeight: {
          [$gt]: this.app.blockchainInfo.tip.height - 500
        },
        inputHeight: null,
        isStake: true
      },
  ...
  async getMatureBalance(ids, transaction) {
    ...
    let result = await TransactionOutput.aggregate('value', 'SUM', {
      where: {
        ...
        blockHeight: {
          [$between]: [1, this.app.blockchainInfo.tip.height - 500]
        },
        inputHeight: null
      },
      transaction
  ...
  async updateRichList() {
    ...
    try {
      ...
      await this.db.sequelize.query(sql`DELETE FROM rich_list`, {
        transaction
      });
      await RichList.bulkCreate(list.map(({
        addressId,
        balance
      }) => ({
        addressId,
