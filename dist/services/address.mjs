export class AddressService {
  ...
  async getAddressSummary(addressIds, p2pkhAddressIds, rawAddresses, transaction) {
    ...
    let [{
      totalReceived,
      totalSent
    }, unconfirmed, staking, mature, qrc20Balances, qrc721Balances, ranking, blocksMined, transactionCount] = await Promise.all([balanceService.getTotalBalanceChanges(addressIds, transaction), balanceService.getUnconfirmedBalance(addressIds, transaction), balanceService.getStakingBalance(addressIds, transaction), balanceService.getMatureBalance(p2pkhAddressIds, transaction), qrc20Service.getAllQRC20Balances(hexAddresses, transaction), qrc721Service.getAllQRC721Balances(hexAddresses, transaction), balanceService.getBalanceRanking(addressIds, transaction), Block.count({
      where: {
        minerId: {
          [$in]: p2pkhAddressIds
        },
  ...
  async getAddressTransactionCount(addressIds, rawAddresses, fromBlock, toBlock, transaction) {
    ...
    const sql = this.sql;
    let topics = rawAddresses.filter(address => address.type === RawAddress.PAY_TO_PUBLIC_KEY_HASH).map(address => Buffer.concat([Buffer.alloc(12), address.data]));
    let [{
      count
  ...
  async getAddressContractTransactionCount(rawAddresses, contract, fromBlock, toBlock, transaction) {
    ...
    let [{
      ...
      WHERE (sender_type, sender_data) IN ${rawAddresses.map(address => [Address.parseType(address.type), address.data])}
        AND ${this.services.block.getRawBlockFilter(fromBlock, toBlock)} AND ${{
      raw: contractFilter
    }}
    `, {
      type: this.db.sequelize.QueryTypes.SELECT,
  ...
  async getAddressQRC20TokenTransactionCount(rawAddresses, token, transaction) {
    ...
    let topicAddresses = rawAddresses.filter(address => address.type === Address.PAY_TO_PUBLIC_KEY_HASH).map(address => Buffer.concat([Buffer.alloc(12), address.data]));
  ...
  async getAddressQRC20TokenMempoolTransactions(rawAddresses, token, transaction) {
    ...
    let hexAddresses = rawAddresses.filter(address => address.type === RawAddress.PAY_TO_PUBLIC_KEY_HASH).map(address => address.data);
    ...
    return await Promise.all(transactions.map(async tx => {
      ...
      return {
        ...
        ...(to && typeof addresses[1] === 'object' ? {
          to: addresses[1].hex.toString('hex'),
          toHex: addresses[1].hex
        } : {
          to: addresses[1]
        }),
        value,
