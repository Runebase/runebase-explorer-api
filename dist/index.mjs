...
async function main() {
  ...
  const redisUrl = config.redis.password ? `redis://:${config.redis.password}@${config.redis.host}:${config.redis.port}/${config.redis.db}` : `redis://${config.redis.host}:${config.redis.port}/${config.redis.db}`;
...
function startAgent(services) {
  ...
  io.on('block', async block => {
    ...
    try {
      ...
      try {
        ...
        logger.error({
          err
        }, 'Failed to update stakeweight');
      }
    } catch (err) {
      logger.error({
        err
      }, 'Failed to create transaction for stakeweight');
    }
  ...
  io.on('mempool-transaction', async id => {
    ...
    try {
      ...
      try {
        ...
        if (txData) {
          let transformed = await services.transaction.transformTransaction(txData, {
            brief: true
          });
          if (namespace) {
          ...
          for (let address of addresses) {
            namespace.to(`address/${address}`).emit('address/transaction', {
              address,
        ...
        logger.error({
          err
        }, 'Failed to process mempool transaction');
      }
    } catch (err) {
      logger.error({
        err
      }, 'Failed to create transaction for mempool');
    }
  });
  io.on('disconnect', () => {
  ...
  async function updateStatistics() {
    if (tip && Buffer.compare(lastTipHash, tip.hash) !== 0) {
      ...
      try {
        ...
        logger.error({
          err
        }, 'Failed to update QRC20 statistics');
      }
      try {
        ...
        try {
          ...
          logger.error({
            err
          }, 'Failed to update daily transactions');
        }
      } catch (err) {
        logger.error({
          err
        }, 'Failed to create transaction for daily transactions');
      }
      try {
        ...
        try {
          ...
          logger.error({
            err
          }, 'Failed to update block interval');
        }
      } catch (err) {
        logger.error({
          err
        }, 'Failed to create transaction for block interval');
      }
      try {
        ...
        try {
          ...
          logger.error({
            err
          }, 'Failed to update address growth');
        }
      } catch (err) {
        logger.error({
          err
        }, 'Failed to create transaction for address growth');
      }
    }
  }
  ...
  setTimeout(async () => {
    try {
      ...
      logger.error({
        err
      }, 'Failed initial stakeweight update');
    }
    try {
...
async function emitBlockTip(tip, services) {
  ...
  try {
    ...
    try {
      ...
      logger.error({
        err
      }, 'Failed to emit block tip');
    }
  } catch (err) {
    logger.error({
      err
    }, 'Failed to create transaction for block tip');
  }
}
main().catch(err => {
  logger.fatal({
    err
  }, 'Failed to start runebase-explorer-api');
  process.exit(1);
});