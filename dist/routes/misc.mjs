...
export default router => {
  ...
  router.get('/misc/rich-list', asyncHandler(async (req, res) => {
    let {
      totalCount,
      list
    } = await services.balance.getRichList(req.state.pagination, req.state.transaction);
    res.json({
      totalCount,
      list: list.map(item => ({
  ...
  router.get('/misc/biggest-miners', asyncHandler(async (req, res) => {
    ...
    let {
      totalCount,
      list
    } = await services.block.getBiggestMiners(lastNBlocks, req.state.pagination, req.state.transaction);
    res.json({
      totalCount,
      list: list.map(item => ({
