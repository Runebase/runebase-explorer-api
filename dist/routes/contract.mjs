...
export default router => {
  router.get('/contract/:contract', asyncHandler(async (req, res) => {
    let summary = await services.contract.getContractSummary(req.state.contract.contractAddress, req.state.contract.addressIds, req.state.fromBlock, req.state.toBlock, req.state.transaction);
    res.json({
  ...
  router.get('/contract/:contract/txs', asyncHandler(async (req, res) => {
    let {
      totalCount,
      transactions
    } = await services.contract.getContractTransactions(req.state.contract.contractAddress, req.state.contract.addressIds, req.state.pagination, req.state.fromBlock, req.state.toBlock, req.state.transaction);
    res.json({
      totalCount,
  ...
  router.get('/contract/:contract/basic-txs', asyncHandler(async (req, res) => {
    let {
      totalCount,
      transactions
    } = await services.contract.getContractBasicTransactions(req.state.contract.contractAddress, req.state.pagination, req.state.fromBlock, req.state.toBlock, req.state.transaction);
    res.json({
      totalCount,
  ...
  router.get('/contract/:contract/balance-history', asyncHandler(async (req, res) => {
    let {
      totalCount,
      transactions
    } = await services.balance.getBalanceHistory(req.state.contract.addressIds, {
      nonZero: true
    }, req.state.pagination, req.state.transaction);
    res.json({
      totalCount,
  ...
  router.get('/contract/:contract/qrc20-balance-history', asyncHandler(async (req, res) => {
    ...
    let {
      totalCount,
      transactions
    } = await services.qrc20.getQRC20BalanceHistory([req.state.contract.contractAddress], tokenAddress, req.state.pagination, req.state.transaction);
    res.json({
      totalCount,
      transactions: transactions.map(tx => ({
        id: tx.id.toString('hex'),
  ...
  router.get('/searchlogs', asyncHandler(async (req, res) => {
    ...
    let {
      totalCount,
      logs
    } = await services.contract.searchLogs({
      contract,
      topic1,
      topic2,
      topic3,
      topic4
    }, req.state.pagination, req.state.fromBlock, req.state.toBlock, req.state.transaction);
    res.json({
      totalCount,
      logs: logs.map(log => ({
