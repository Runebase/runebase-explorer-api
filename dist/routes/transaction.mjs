...
export default router => {
  const txListHandler = asyncHandler(async (req, res) => {
    ...
    let {
      totalCount,
      ids
    } = await services.transaction.getAllTransactions(address, req.state.pagination, req.state.transaction);
  ...
  router.get('/tx/:id', asyncHandler(async (req, res) => {
    ...
    res.json(await services.transaction.transformTransaction(transaction, {
      brief
    }));
  }));
  router.get('/txs/:ids', asyncHandler(async (req, res) => {
    ...
    let brief = 'brief' in req.query;
    let transactions = await Promise.all(ids.map(id => services.transaction.getTransaction(Buffer.from(id, 'hex'), req.state.transaction)));
    if (!transactions.every(Boolean)) return res.status(404).end();
    res.json(await Promise.all(transactions.map(tx => services.transaction.transformTransaction(tx, {
      brief
    }))));
  }));
